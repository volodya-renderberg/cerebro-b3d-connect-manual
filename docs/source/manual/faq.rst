...
Не получается сделать коммит, возникает сообщение об ошибке
------------------------------------------------------------

.. image:: ./../_static/images/connection_error_message.png


В последней строке сообщения пишется тип ошибки, в данном случае ``Exception: Connection error: ...`` что означает потерю связи.

Надо просто закрыть десктоп версию серебры, перезапустить блендер и повторить процедуру, которую не удалось выполнить ранее (коммит или что ещё).