...
* Используем констрейн **Child Of**.
    ...
    * обязательно нужны ключи на включение этого констрейна, даже если взаимодействие начинается сразу в первом кадре.
        ...
        * основная суть этих ключей:

            * содаются два ключа в соседних кадрах на включение или отключение констрейна параметр ``influence`` 0-1 или 1-0.
            * создаются ключи на сам контрол в фактическом его метсо расположении, оно может отличаться от значения в ``location``.

        * Если пользуетесь другими инструментами такими как *Dynamic Parent* или подобными, убедитесь что анимация созданная таким образом восстанавливается :ref:`build-scene-page` или :ref:`import_animation_from_incoming_task`.
